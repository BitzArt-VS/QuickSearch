...
/// <para>
/// Mouse handling follows the framework convention: <see cref="GuiButton"/> declares an inner
/// <see cref="GuiContainer"/> that fills its content area and registers slot-level
/// <c>OnMouseDown</c> / <c>OnMouseUp</c> / <c>OnMouseClick</c> handlers via the render-tree
/// builder. The button itself does <i>not</i> expose virtual mouse hooks — only a single
...
public sealed class GuiButton : GuiComponent
{
    ...
    /// <summary>Solid background fill colour. Default <see cref="GuiVanillaStyle.ButtonBackColor"/>.</summary>
    public GuiColor BackgroundColor { get; set; } = GuiVanillaStyle.ButtonBackColor;

    // Press / hover state — driven by inner GuiContainer's mouse handlers, read by Render.
    // The renderer dispatches mouse events for this slot directly into HandleMouseDown/Up/Enter/Leave,
    // so these fields are mutated on the render thread alongside Render — no synchronisation
    // is required. The renderer also invalidates its cached surface after dispatching any
    // mouse event, so the next frame redraws and picks up the new values without
    // requiring StateHasChanged (which would force a full reconcile per click).
    ...
    public override GuiSize Measure(double availableWidth, double availableHeight)
    {
        var ts = NormalFont.Measure(Text);
        return new GuiSize(ts.Width + HorizontalTextPadding * 2, ts.Height + VerticalTextPadding * 2);
    }
    ...
    {
        // Inner click target: a transparent container that fills the button's content area.
        // The button's own LayoutParameters.Padding is zero by default, so this container's
        // bounds match the button's bounds exactly — a click anywhere on the visible button
        // hits this region.
        // Absolute positioning excludes this container from MeasureChildren so the button's
        // own FitContent height measurement is not inflated to the full parent available height.
    ...
    private void HandleMouseDown(GuiMouseEventArgs e)
    {
        ...
        // No StateHasChanged required — DialogRenderer marks the surface dirty after every
        // mouse dispatch, so the next frame already picks up the new _isPressed value.
    }
    ...
    private void HandleMouseUp(GuiMouseEventArgs e)
    {
        ...
        bool inside = e.X >= _lastBounds.X && e.X < _lastBounds.Right
