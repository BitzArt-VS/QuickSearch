...
public class GuiDialogCloseIcon : GuiComponent
{
    ...
    /// <summary>Idle-state cross colour. Default <see cref="GuiVanillaStyle.DialogDefaultTextColor"/>.</summary>
    ...
    // Hover state — flipped by the inner click target's OnMouseEnter/Leave handlers and
    // read by Render. The renderer marks the surface dirty whenever a hover handler runs,
    // so the next frame redraws automatically — no StateHasChanged needed.
    ...
    {
        // Inner mouse target — fills the icon's allocated bounds. Absolute positioning so
        // the icon's own FitContent measurement isn't affected by this slot. Same pattern
        // as GuiButton's inner click target.
    ...
    private void HandleMouseEnter(GuiMouseEventArgs e) => _isHovered = true;
    private void HandleMouseLeave(GuiMouseEventArgs e) => _isHovered = false;
