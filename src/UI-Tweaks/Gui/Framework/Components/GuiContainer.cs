...
public class GuiContainer : GuiComponent
{
    ...
    /// <summary>
    /// Axes on which scrolling is enabled. Defaults to <see cref="GuiScroll.None"/>.
    ...
    /// </summary>
    public GuiScroll Scroll { get; set; } = GuiScroll.None;

    /// <summary>
    /// Axes for which a scrollbar may be displayed. Defaults to <see cref="GuiScroll.Both"/>.
    ...
    /// </summary>
    public GuiScroll Scrollbar { get; set; } = GuiScroll.Both;

    /// <summary>
    /// Axes for which the scrollbar should remain visible even when content does not
    /// overflow. Defaults to <see cref="GuiScroll.None"/>. Has no effect for axes not
    /// included in <see cref="Scroll"/> / <see cref="Scrollbar"/>.
    /// </summary>
    public GuiScroll AlwaysShowScrollbar { get; set; } = GuiScroll.None;
    ...
    internal GuiScroll EffectiveScroll;
    ...
    {
        if ((EffectiveScroll & GuiScroll.Vertical) != 0)
            ScrollY = Clamp(ScrollY - deltaPrecise * WheelStep, 0, Math.Max(0, _contentH - _viewportH));
        else if ((EffectiveScroll & GuiScroll.Horizontal) != 0)
    ...
    private void HandleHScrollbarDown(GuiMouseEventArgs e)
    {
        ...
        var (handleX, handleW) = GetHHandleSpan();
        if (e.X >= handleX && e.X < handleX + handleW)
        {
            _hDragHandleOffset = e.X - handleX;
        }
        else
        {
            _hDragHandleOffset = handleW / 2.0;
            ApplyHHandlePos(e.X - _hDragHandleOffset);
        }
        _hDragging = true;
    }
