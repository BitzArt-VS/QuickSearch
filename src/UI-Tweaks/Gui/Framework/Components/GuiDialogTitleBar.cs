...
public class GuiDialogTitleBar : GuiContainer
{
    ...
    // Screen-space anchor for drag delta computation. Stored in physical pixels because the
    // title bar moves with its parent dialog mid-drag, which would invalidate any local
    // (logical-bounds-relative) reference frame between events.
    private int _dragLastScreenX;
    ...
    protected override void BuildRenderTree(IGuiRenderTreeBuilder builder)
    {
        ...
        {
            builder.Add<GuiContainer>(int.MaxValue - 1, fill: true, positioning: GuiComponentPositioning.Absolute)
    ...
    private void HandleMouseDown(GuiMouseEventArgs e)
    {
        ...
        _dragging = true;
        _dragLastScreenX = e.ScreenX;
        _dragLastScreenY = e.ScreenY;
    }

    private void HandleMouseMove(GuiMouseEventArgs e)
    {
        if (!_dragging) return;
        // Convert physical-pixel screen delta to logical pixels. The dialog's offset is in
        // logical pixels (so it remains stable across UI-scale changes), and the renderer
        // multiplies by GUIScale to find the on-screen blit position.
        float scale = RuntimeEnv.GUIScale;
        double dx = (e.ScreenX - _dragLastScreenX) / scale;
        double dy = (e.ScreenY - _dragLastScreenY) / scale;
        _dragLastScreenX = e.ScreenX;
        _dragLastScreenY = e.ScreenY;
