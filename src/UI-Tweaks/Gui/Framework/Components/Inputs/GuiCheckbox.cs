...
public sealed class GuiCheckbox : GuiInputBase
{
    ...
    /// to <see cref="GuiVanillaStyle.SwitchMarkColor"/> — the approximation of what vanilla's
    ...
    public GuiCheckbox()
    {
        // Square by default — the layout pass takes Size as both width and height through
        // OnParametersSet so that explicit Width / Height overrides still work.
        LayoutParameters.WidthMode = GuiSizeMode.FitContent;
        LayoutParameters.HeightMode = GuiSizeMode.FitContent;
    }

    /// <inheritdoc/>
    public override GuiSize Measure(double availableWidth, double availableHeight)
    {
        return new GuiSize(Size, Size);
    }
