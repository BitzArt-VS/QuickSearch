...
internal static class ClientApiExtensions
{
    ...
    /// <summary>
    /// Registers a custom mouse cursor with the game's platform layer. Wraps
    /// <c>ClientPlatformAbstract.LoadMouseCursor</c>, which is not exposed via
    /// <see cref="ICoreClientAPI"/>; reached via the same reflection chain as
    /// <see cref="UnregisterDialog"/> (clientApi → 'game' (ClientMain) → 'Platform').
    /// Returns false on any failure (cursor too large, exception during load); the game's
    /// implementation already logs failures internally.
    /// </summary>
